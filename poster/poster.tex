...
function $v_\pi(s) = \mathop{\mathbb{E}}[R] = \mathop{\mathbb{E}}[\sum_{t=0}^\infty \gamma^t R_t | S_0 = s]$
...
The main contribution of the paper~\cite{TDGamma} is introducing the
$\text{TD}(\gamma)$ family of algorithms, which eliminates the $\lambda$
parameter. It is shown that $\text{TD}(\gamma)$ outperforms current
state-of-the-art algorithms, such as $\text{TD}(\lambda)$.

$\text{TD}(\gamma)$ algorithm is the simplest algorithm of the
$\text{TD}_{\gamma}$ family. Given a set of trajectories and a discount factor,
it estimates $V_\pi(S)$.
...
approximates $q_\pi(w, a) = \mathop{\mathbb{E}}[R] =
\mathop{\mathbb{E}}[\sum_{t=0}^\infty R_t | S_t = s, A_t = a]$,
$\text{TD}(\gamma)$ can be used to improve the results of state-of-the-art
algorithms. By definition, $v_\pi(s) = \sum_{a \in \mathcal{A}} q_\pi(s, a)$
and hence knowing the dynamics of the environment one could efficiently
navigate it using value function. However, in most cases the transitional
probabilities are not known to the agent. More sophisticated methods like
model-based Reinforcement Learning are able to deal with that issue. Despite
the additional complexity, such methods show good performance in cases where
thorough planning is crucial for the success of chosen approach. A great
example of such problem would be Sokoban~\cite{Sokoban} where researchers used
the environment model to approximate model dynamics.
...
\begin{figure}%
    \centering
    \subfloat[$\text{TD}({\lambda})$]{{\includegraphics[width=0.45\textwidth]{figures/random-walk-td-lambda.png}}}
    \qquad
    \subfloat[$\text{TD}({\gamma})$]{{\includegraphics[width=0.45\textwidth]{figures/random-walk-td-lambda.png}}}
...
Colab}. The experiments are based on  code with further intention to contribute
the results to main
\href{https://github.com/ShangtongZhang/reinforcement-learning-an-introduction}{reposotiry}
of Python implementation of "Reinforcement Learning: An
Introduction"~\cite{Sutton}.
