...
\begin{figure}%
    \centering
    \subfloat[$\text{TD}({\lambda})$ for different settings of $\lambda$]{{\includegraphics[width=0.46\textwidth]{figures/td_lambda_50.png}}}
    \qquad
    \subfloat[$\text{TD}({\gamma})$ for different settings of $\gamma$]{{\includegraphics[width=0.46\textwidth]{figures/td_gamma_50.png}}}
...
An important subset of value approximation techniques based on Temporal
Difference Learning was studied in this project.  By running algorithm on a
small task, it was shown that novel $\text{TD}(\gamma)$ algorithm is able to
outperform its commonly used precursor. Few key improvements are yet to be
made: $\text{TD}_{\gamma}(C)$ is another algorithm introduced by the studied
paper, which can be improved by introducing automated ways to choose $C$ and
reducing the hyperparameter search space. $\text{TD}(\gamma)$ can be studied in
more challenging settings and environments in order to test its feasibility in
...
\bibitem{Code}
  \href{https://drive.google.com/file/d/1fmRALZn0-BRKlHWK\_juP9sswUdpEtCZ0/view?usp=sharing}{Google Colab} ---
  IPython Notebook with experiments based on Shangtong Zhang's work,
  \href{https://github.com/omtcvxyz/optimization-class-project}{omtcvxyz/optimization-class-project} --- supplementary materials.
